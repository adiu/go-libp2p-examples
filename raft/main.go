...
import (
	...
	libraft "github.com/libp2p/go-libp2p-raft"
...
func main() {
	...
	// cluster.
	consensus1 := libraft.NewConsensus(&raftState{3})
	consensus2 := libraft.NewConsensus(&raftState{3})
	consensus3 := libraft.NewConsensus(&raftState{3})

	// Create LibP2P transports Raft
	transport1, err := libraft.NewLibp2pTransport(peer1, time.Minute)
	if err != nil {
		fmt.Println(err)
		return
	}
	transport2, err := libraft.NewLibp2pTransport(peer2, time.Minute)
	if err != nil {
		fmt.Println(err)
		return
	}
	transport3, err := libraft.NewLibp2pTransport(peer3, time.Minute)
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	actor1 := libraft.NewActor(raft1)
	actor2 := libraft.NewActor(raft2)
	actor3 := libraft.NewActor(raft3)
	...
	updateState := func(c *libraft.Consensus) {
		nUpdates := 0
		for {
			if nUpdates >= 1000 {
				break
