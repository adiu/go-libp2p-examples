...
`
		...
		"put": func(args ...string) (interface{}, error) {
			...
			if err := node.PutValue(context.Background(), fmt.Sprintf("/adiu/%s", key), []byte(value)); err != nil {
		...
		"get": func(args ...string) (interface{}, error) {
			...
			key := args[0]
			buf, err := node.GetValue(context.Background(), fmt.Sprintf("/adiu/%s", key))
			if err != nil {
	...
	app.Commands = []cli.Command{
		{
			...
			Action: func(ctx *cli.Context) error {
				...
				fmt.Println("source\t: https://github.com/adiu")
				return nil
			},
		},
		{
